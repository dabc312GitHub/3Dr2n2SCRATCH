...
class Dataset(data.Dataset):
    ...
    def __init__(self, root, transform=None, loader_image=loader_image, loader_label=loader_label,
                 model_portion=[0, 0.8], min_views=1, max_views=5, batch_size=24):
        image_dict = {}
        image_list = []
        cat_model_list = []
        im_list = []
        ...
        for directory in main_dir_list:  # loop over model-categories
            ...
            for subdirectory in model_list:  # loop over models
                ...
                # casoERROR = os.listdir(os.path.join(image_root, directory, 'rendering')
                # print(sub_dir_list)
                # exit()
    ...
    def __getitem__(self, index):
        ...
        try:
            labeltmp = self.loader_label(
                os.path.join(self.label_root, self.cat_model_list[index][0], self.cat_model_list[index][1],
                             'model.binvox'))
            label = torch.from_numpy(labeltmp.data.astype('uint8')).long()
            for view in range(self.cur_n_views):
                imgtmp = self.loader_image(
                    os.path.join(self.image_root, self.cat_model_list[index][0], self.cat_model_list[index][1],
                                 'rendering', filenames[view]))
