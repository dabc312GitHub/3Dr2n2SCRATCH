...
if not 'val_set' in locals():
    print('Reading image info from disk...')
    t1_ImageFolder = time.time()
    val_set = dataset.Dataset(root=database_path, transform=val_transform, model_portion=[0.8, 1], min_views=1,
                              max_views=max_views, batch_size=batch_size)
    t2_ImageFolder = time.time()
